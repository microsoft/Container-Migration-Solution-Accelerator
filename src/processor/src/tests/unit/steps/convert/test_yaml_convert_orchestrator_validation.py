...
def test_yaml_convert_orchestrator_rejects_none_task_param():
    async def _run():
        orch = YamlConvertOrchestrator(app_context=_FakeAppContext())
...
def test_yaml_convert_orchestrator_requires_process_id():
    async def _run():
        orch = YamlConvertOrchestrator(app_context=_FakeAppContext())
        msg = Design_ExtendedBooleanResult(process_id=None)
