...
// ── Container Image Parameters (azd-supplied) ──
...
// Default placeholder image for azd initial deploy
...
// Its AcrPull role is granted up-front (no dependency on the apps), which avoids
// the chicken-and-egg deadlock that occurs when a system-assigned identity is used
// for the registry: the app cannot finish provisioning without AcrPull, but the
// AcrPull grant cannot be created without the app's system-assigned principalId.
module acrPullIdentity './bicep/modules/identity/managed-identity.bicep' = {
  name: take('module.acr-pull-identity.${solutionName}', 64)
  params: {
    solutionName: solutionSuffix
    identityName: take('id-acrpull-${solutionSuffix}', 128)
    location: solutionLocation
    tags: union(existingTags, tags, { TemplateName: 'Container Migration' })
  }
  scope: resourceGroup(resourceGroup().name)
...
module acrPullRoleAssignment './bicep/modules/identity/role-assignments.bicep' = {
  name: take('module.acr-pull-role.${solutionName}', 64)
  params: {
    solutionName: solutionSuffix
    acrResourceId: containerRegistry.outputs.resourceId
    acrPullIdentityPrincipalId: acrPullIdentity.outputs.principalId
  }
  scope: resourceGroup(resourceGroup().name)
}
...
module ca_backend_api './bicep/modules/compute/container-app.bicep' = {
  ...
  params: {
    ...
    location: solutionLocation
    tags: union(existingTags, tags, { TemplateName: 'Container Migration', 'azd-service-name': 'backend' })
    environmentResourceId: containerAppEnv.outputs.resourceId
    ingressExternal: true
    ingressTargetPort: 80
    managedIdentities: { systemAssigned: true, userAssignedResourceIds: [ acrPullIdentity.outputs.resourceId ] }
    registries: [
      {
        server: containerRegistry.outputs.loginServer
        identity: acrPullIdentity.outputs.resourceId
      }
    ]
    containers: [
      {
        name: 'backend-api'
        image: !empty(backendImageName) ? backendImageName : defaultImage
        env: [
...
module ca_frontend './bicep/modules/compute/container-app.bicep' = {
  ...
  params: {
    ...
    location: solutionLocation
    tags: union(existingTags, tags, { TemplateName: 'Container Migration', 'azd-service-name': 'frontend' })
    environmentResourceId: containerAppEnv.outputs.resourceId
    ingressExternal: true
    ...
    containers: [
      {
        name: 'frontend'
        image: !empty(frontendImageName) ? frontendImageName : defaultImage
        env: [
...
module ca_processor './bicep/modules/compute/container-app.bicep' = {
  ...
  params: {
    ...
    location: solutionLocation
    tags: union(existingTags, tags, { TemplateName: 'Container Migration', 'azd-service-name': 'processor' })
    environmentResourceId: containerAppEnv.outputs.resourceId
    ...
    containers: [
      {
        name: 'processor'
        image: !empty(processorImageName) ? processorImageName : defaultImage
        env: [
          { name: 'AZURE_OPENAI_ENDPOINT', value: aiFoundryEndpoint }
          { name: 'AZURE_OPENAI_CHAT_DEPLOYMENT_NAME', value: gptModelName }
          { name: 'AZURE_OPENAI_EMBEDDING_DEPLOYMENT_NAME', value: embeddingModel }
...
module role_assignments './bicep/modules/identity/role-assignments.bicep' = {
  ...
  params: {
    ...
    frontendAppServicePrincipalId: ca_frontend.outputs.principalId
    acrResourceId: containerRegistry.outputs.resourceId
...
@description('The login server endpoint of the Azure Container Registry (used by azd for image push).')
